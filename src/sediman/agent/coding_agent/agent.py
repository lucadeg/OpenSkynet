...
class CodingAgent:
    def __init__(
        ...
        self._verifier: VerifyLoop | None = None
    ...
    async def run(self, task: str) -> CodingResult:
        ...
        try:
            ...
            if self._edited_files and self._verify_after_edits:
                self._emit_step("coding: verifying", detail=f"{len(self._edited_files)} file(s) edited")
                verify_result = await self._run_verification()
                verifications_passed = sum(1 for r in verify_result if r.success)
                verifications_failed = len(verify_result) - verifications_passed
        ...
        success = "failed" not in result_text.lower()[:50] and len(errors_encountered) == 0
    ...
    async def _run_verification(self) -> list[Any]:
        if self._verifier is None:
            self._verifier = VerifyLoop(
                project_info=self.project,
                on_progress=self._on_step,
            )

        for f in self._edited_files:
            self._verifier.track_edit(f)

        try:
            results = await self._verifier.verify(aggressive=False)
            if results:
                for r in results:
